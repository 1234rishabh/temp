...
async def drain_to_stall(dut, max_cycles=40):
    """
    Continuously present grant_valid (via req) with out_ready=1
    until out_valid stops being asserted (credit stall).
    ...
    for _ in range(max_cycles):
        ...
        if int(dut.out_valid.value) and int(dut.out_ready.value):
            dispatched += 1
        # Credit stall: out_valid went low and no new transaction started
        if dispatched > 0 and not int(dut.out_valid.value):
            # Confirm it stays low (not a bubble)
            await RisingEdge(dut.clk)
...
async def tc_D01_credit_drain_halts_output(dut):
    ...
    for _ in range(40):
        ...
        if int(dut.out_valid.value) and int(dut.out_ready.value):
            dispatched += 1
        if dispatched > 0 and not int(dut.out_valid.value):
            # Confirm stall holds for 2 more cycles
            await RisingEdge(dut.clk)
            await RisingEdge(dut.clk)
...
async def tc_D03_simultaneous_return_and_consume(dut):
    ...
    # Must keep dispatching for at least 2 more cycles
    stalled_early = False
    for _ in range(3):
        await RisingEdge(dut.clk)
        # A stall here with no backpressure = underflow
        if not int(dut.out_valid.value) and int(dut.out_ready.value):
            stalled_early = True
    assert not stalled_early, \
        "Credit underflow: output stopped after simultaneous return+consume"
